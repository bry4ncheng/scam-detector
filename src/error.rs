...
use axum::{
    ...
    Json, extract::rejection::{JsonRejection, QueryRejection},
};
...
/// This error enum will be used heavily by the controller and services layer to return errors
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        let (status_code, error_message, status_text) = match self {
            AppError::Unauthorized(err) => (StatusCode::UNAUTHORIZED, err, "Unauthorized"),
            AppError::Forbidden => (StatusCode::FORBIDDEN, AppError::Forbidden.to_string(), "Forbidden"),
            AppError::ForbiddenWithMessage(err) => (StatusCode::FORBIDDEN, err, "Forbidden"),
            ...
            AppError::ValidationError(err) => (StatusCode::BAD_REQUEST, err.to_string(), "Bad Request"),
            AppError::ValidationErrors(err) => (StatusCode::BAD_REQUEST, err.to_string(), "Bad Request"),
            AppError::JsonExtractorRejection(err) => (StatusCode::BAD_REQUEST, err.to_string(), "Bad Request"),
            AppError::QueryExtractorRejection(err) => (StatusCode::BAD_REQUEST, err.to_string(), "Bad Request"),
